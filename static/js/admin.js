...
function initializeEventListeners() {
    ...
    const brandForm = document.getElementById('brand-content-form');
    if (brandForm) {
        brandForm.addEventListener('submit', handleBrandContentSubmit);
        brandForm.addEventListener('input', updateBrandPreviewFromForm);
    }

    const partnersForm = document.getElementById('partners-content-form');
    if (partnersForm) {
        partnersForm.addEventListener('submit', handlePartnersContentSubmit);
        partnersForm.addEventListener('input', updatePartnersPreviewFromForm);
    }

    const servicePagesForm = document.getElementById('service-pages-content-form');
    if (servicePagesForm) {
        servicePagesForm.addEventListener('submit', handleServicePagesContentSubmit);
        servicePagesForm.addEventListener('input', updateServicePagesPreviewFromForm);
    }
    ...
    if (partnersList) {
        partnersList.addEventListener('click', handlePartnersListClick);
        partnersList.addEventListener('input', updatePartnersPreviewFromForm);
        partnersList.addEventListener('change', updatePartnersPreviewFromForm);
        registerDragAndDrop('partners-list', '.partner-item', updatePartnersPreviewFromForm);
    }

    const servicePagesList = document.getElementById('service-pages-list');
    if (servicePagesList) {
        servicePagesList.addEventListener('click', handleServicePagesListClick);
        servicePagesList.addEventListener('input', updateServicePagesPreviewFromForm);
        servicePagesList.addEventListener('change', updateServicePagesPreviewFromForm);
        registerDragAndDrop('service-pages-list', '.service-page-item', updateServicePagesPreviewFromForm);
    }
    ...
    const brandUploadBtn = document.getElementById('brand-logo-upload');
    if (brandUploadBtn) {
    ...
    if (brandLogoFile) {
        brandLogoFile.addEventListener('change', handleBrandLogoUpload);
    }
    ...
    document.addEventListener('change', (event) => {
        ...
        if (event.target && event.target.matches('[data-role="partner-logo-input"]')) {
            handlePartnerLogoUpload(event);
        }
...
async function loadContentData(tabName = null) {
    const contentMap = {
        ...
        partners: {
            ...
            section: 'partners',
            cacheKey: 'partners',
            handler: populatePartnersForm
        },
        'service-pages': {
            page: 'services',
            section: 'pages',
            cacheKey: 'servicePages',
            handler: populateServicePagesForm
        },
        services: {
            page: 'home',
            section: 'services',
...
function populateBrandForm(content) {
    const nameInput = document.getElementById('brand-name');
    const logoInput = document.getElementById('brand-logo-url');

    if (nameInput) nameInput.value = content.company_name || 'Gautam Insurance';
    if (logoInput) logoInput.value = content.logo_url || '';

    updateBrandPreviewFromForm();
}

function populatePartnersForm(content) {
    const titleInput = document.getElementById('partners-title');
    const subtitleInput = document.getElementById('partners-subtitle');

    if (titleInput) titleInput.value = content.title || '';
    if (subtitleInput) subtitleInput.value = content.subtitle || '';

    renderPartnersEditor(Array.isArray(content.partners) ? content.partners : []);
    updatePartnersPreviewFromForm();
}

function populateServicePagesForm(content) {
    const titleInput = document.getElementById('service-pages-title');
    const subtitleInput = document.getElementById('service-pages-subtitle');

    if (titleInput) titleInput.value = content.title || '';
    if (subtitleInput) subtitleInput.value = content.subtitle || '';

    renderServicePagesEditor(Array.isArray(content.pages) ? content.pages : []);
    updateServicePagesPreviewFromForm();
}
...
function renderPartnersEditor(partners) {
    const partnersList = document.getElementById('partners-list');
    ...
    safePartners.forEach((partner) => addPartnerItem(partner));

    const actions = document.createElement('div');
    actions.className = 'list-actions';
    actions.innerHTML = '<button type="button" class="btn btn-secondary" data-action="add-partner">Add New Company</button>';
    partnersList.appendChild(actions);
}

function renderServicePagesEditor(pages) {
    const pagesList = document.getElementById('service-pages-list');
    if (!pagesList) return;

    pagesList.innerHTML = '';

    const safePages = pages.length ? pages : [{ title: '', slug: '', summary: '', description: '', image_url: '', highlights: [], cta_text: '', cta_link: '' }];
    safePages.forEach((page) => addServicePageItem(page));

    const actions = document.createElement('div');
    actions.className = 'list-actions';
    actions.innerHTML = '<button type="button" class="btn btn-secondary" data-action="add-service-page">Add Service Page</button>';
    pagesList.appendChild(actions);
}
    ...
    item.innerHTML = `
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="text" data-field="slug" value="${escapeHtml(service.slug)}" placeholder="home-insurance">
            </div>
        </div>
        <div class="form-group">
...
function addPartnerItem(partner = {}) {
    const partnersList = document.getElementById('partners-list');
    if (!partnersList) return;

    const item = document.createElement('div');
    item.className = 'partner-item draggable-item';
    item.setAttribute('draggable', 'true');
    item.innerHTML = `
        <div class="drag-handle" title="Drag to reorder">
            <i class="fas fa-grip-vertical"></i>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" data-field="name" value="${escapeHtml(partner.name)}" required>
            </div>
            <div class="form-group">
                <label>Logo URL</label>
                <input type="text" data-field="logo_url" value="${escapeHtml(partner.logo_url)}" placeholder="/uploads/images/company-logo.png">
            </div>
        </div>
        <div class="form-group">
            <button type="button" class="btn btn-secondary" data-action="upload-partner-logo">
                Upload Logo
            </button>
            <input type="file" accept="image/*" class="partner-logo-file" data-role="partner-logo-input" style="display: none;">
        </div>
        <div class="form-group">
            <label>Website (optional)</label>
            <input type="text" data-field="website" value="${escapeHtml(partner.website)}" placeholder="https://example.com">
        </div>
        <button type="button" class="btn btn-danger" data-action="remove-partner">Delete Company</button>
        <hr>
    `;

    const actions = partnersList.querySelector('.list-actions');
    if (actions) {
        partnersList.insertBefore(item, actions);
    } else {
        partnersList.appendChild(item);
    }
}

function addServicePageItem(page = {}) {
    const pagesList = document.getElementById('service-pages-list');
    if (!pagesList) return;

    const item = document.createElement('div');
    item.className = 'service-page-item draggable-item';
    item.setAttribute('draggable', 'true');
    item.innerHTML = `
        <div class="drag-handle" title="Drag to reorder">
            <i class="fas fa-grip-vertical"></i>
        ...
        <div class="form-row">
            <div class="form-group">
                <label>Title</label>
                <input type="text" data-field="title" value="${escapeHtml(page.title)}" required>
            </div>
            <div class="form-group">
                <label>Slug</label>
                <input type="text" data-field="slug" value="${escapeHtml(page.slug)}" placeholder="home-insurance">
            </div>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>Summary</label>
                <input type="text" data-field="summary" value="${escapeHtml(page.summary)}" placeholder="Short overview for cards">
            </div>
            <div class="form-group">
                <label>Image URL</label>
                <input type="text" data-field="image_url" value="${escapeHtml(page.image_url)}" placeholder="/uploads/images/service.png">
            </div>
        </div>
        <div class="form-group">
            <label>Description</label>
            <textarea data-field="description" rows="3" placeholder="Full description for the service page">${escapeHtml(page.description)}</textarea>
        </div>
        <div class="form-group">
            <label>Highlights (one per line)</label>
            <textarea data-field="highlights" rows="3">${escapeHtml(Array.isArray(page.highlights) ? page.highlights.join('\n') : '')}</textarea>
        </div>
        <div class="form-row">
            <div class="form-group">
                <label>CTA Text</label>
                <input type="text" data-field="cta_text" value="${escapeHtml(page.cta_text)}" placeholder="Get a Quote">
            </div>
            <div class="form-group">
                <label>CTA Link</label>
                <input type="text" data-field="cta_link" value="${escapeHtml(page.cta_link)}" placeholder="index.html#quote">
            </div>
        </div>
        <button type="button" class="btn btn-danger" data-action="remove-service-page">Remove Page</button>
        <hr>
    `;

    const actions = pagesList.querySelector('.list-actions');
    if (actions) {
        pagesList.insertBefore(item, actions);
    } else {
...
function handlePartnersListClick(event) {
    const button = event.target.closest('button[data-action]');
    if (!button) return;

    if (button.dataset.action === 'add-partner') {
        addPartnerItem();
        updatePartnersPreviewFromForm();
        return;
    }

    if (button.dataset.action === 'upload-partner-logo') {
        const item = button.closest('.partner-item');
        const fileInput = item?.querySelector('[data-role="partner-logo-input"]');
        if (fileInput) {
            fileInput.click();
        }
        return;
    ...
    if (button.dataset.action === 'remove-partner') {
        const item = button.closest('.partner-item');
        if (item) item.remove();
        updatePartnersPreviewFromForm();
    }
...
function handleServicePagesListClick(event) {
    const button = event.target.closest('button[data-action]');
    if (!button) return;

    if (button.dataset.action === 'add-service-page') {
        addServicePageItem();
        updateServicePagesPreviewFromForm();
        return;
    }

    if (button.dataset.action === 'remove-service-page') {
        const item = button.closest('.service-page-item');
        if (item) item.remove();
        updateServicePagesPreviewFromForm();
    }
}
...
function collectServicesFromForm() {
    ...
    document.querySelectorAll('#services-list .service-item').forEach((item) => {
        ...
        let slug = item.querySelector('[data-field="slug"]')?.value?.trim() || '';
...
function collectPartnersFromForm() {
    const partners = [];
    document.querySelectorAll('#partners-list .partner-item').forEach((item) => {
        const name = item.querySelector('[data-field="name"]')?.value?.trim() || '';
        const logoUrl = item.querySelector('[data-field="logo_url"]')?.value?.trim() || '';
        const website = item.querySelector('[data-field="website"]')?.value?.trim() || '';
...
function collectServicePagesFromForm() {
    const pages = [];
    document.querySelectorAll('#service-pages-list .service-page-item').forEach((item) => {
        const title = item.querySelector('[data-field="title"]')?.value?.trim() || '';
        let slug = item.querySelector('[data-field="slug"]')?.value?.trim() || '';
        const summary = item.querySelector('[data-field="summary"]')?.value?.trim() || '';
        const description = item.querySelector('[data-field="description"]')?.value?.trim() || '';
        const imageUrl = item.querySelector('[data-field="image_url"]')?.value?.trim() || '';
        const highlightsRaw = item.querySelector('[data-field="highlights"]')?.value || '';
        const ctaText = item.querySelector('[data-field="cta_text"]')?.value?.trim() || '';
        const ctaLink = item.querySelector('[data-field="cta_link"]')?.value?.trim() || '';
        ...
        if (title || summary || description || imageUrl || highlights.length || ctaText || ctaLink || slug) {
            pages.push({
                title,
                slug,
                summary,
...
async function handleBrandContentSubmit(event) {
    event.preventDefault();

    const content = {
        company_name: document.getElementById('brand-name')?.value?.trim() || '',
        logo_url: document.getElementById('brand-logo-url')?.value?.trim() || ''
    };

    const updated = await updateContent('global', 'brand', content);
    if (updated?.content) {
        contentCache.brand = updated.content;
    }
}

async function handlePartnersContentSubmit(event) {
    event.preventDefault();

    const content = {
        title: document.getElementById('partners-title')?.value?.trim() || '',
        subtitle: document.getElementById('partners-subtitle')?.value?.trim() || '',
        partners: collectPartnersFromForm()
    };

    const updated = await updateContent('home', 'partners', content);
    if (updated?.content) {
        contentCache.partners = updated.content;
    }
}

async function handleServicePagesContentSubmit(event) {
    event.preventDefault();

    const content = {
        title: document.getElementById('service-pages-title')?.value?.trim() || '',
        subtitle: document.getElementById('service-pages-subtitle')?.value?.trim() || '',
        pages: collectServicePagesFromForm()
    };

    const updated = await updateContent('services', 'pages', content);
    if (updated?.content) {
        contentCache.servicePages = updated.content;
    }
}
...
function updateBrandPreviewFromForm() {
    const name = document.getElementById('brand-name')?.value?.trim() || 'Gautam Insurance';
    const logoUrl = document.getElementById('brand-logo-url')?.value?.trim() || '';

    const nameEl = document.getElementById('preview-brand-name');
    const logoEl = document.getElementById('preview-brand-logo');

    if (nameEl) nameEl.textContent = name;
    if (logoEl) {
        if (logoUrl) {
            logoEl.innerHTML = `<img src="${escapeHtml(logoUrl)}" alt="${escapeHtml(name)} logo">`;
        } else {
            const initials = getPartnerInitials(name);
            logoEl.innerHTML = `<span class="preview-brand-fallback">${escapeHtml(initials)}</span>`;
        }
    }
}

function updatePartnersPreviewFromForm() {
    const title = document.getElementById('partners-title')?.value?.trim() || 'We support the following company';
    const subtitle = document.getElementById('partners-subtitle')?.value?.trim() || '';
    const partners = collectPartnersFromForm();

    const titleEl = document.getElementById('preview-partners-title');
    const subtitleEl = document.getElementById('preview-partners-subtitle');
    const listEl = document.getElementById('preview-partners-list');
    ...
    if (!partners.length) {
        listEl.innerHTML = '<div class="preview-empty">No company added yet.</div>';
        return;
    }

    listEl.innerHTML = partners.map((partner) => {
        const name = escapeHtml(partner.name || 'Company');
        const website = escapeHtml(partner.website || '');
        const initials = getPartnerInitials(partner.name || 'Company');
        const logo = partner.logo_url
            ? `<img src="${escapeHtml(partner.logo_url)}" alt="${name} logo">`
            : `<div class="preview-partner-fallback">${initials}</div>`;

        return `
            <div class="preview-partner-card">
                <div class="preview-partner-logo">${logo}</div>
                <div>
                    <strong>${name}</strong>
                    ${website ? `<div class="preview-partner-url">${website}</div>` : ''}
                </div>
            </div>
        `;
    }).join('');
...
function updateServicePagesPreviewFromForm() {
    const title = document.getElementById('service-pages-title')?.value?.trim() || 'Service Pages';
    const subtitle = document.getElementById('service-pages-subtitle')?.value?.trim() || '';
    const pages = collectServicePagesFromForm();

    const titleEl = document.getElementById('preview-service-pages-title');
    const subtitleEl = document.getElementById('preview-service-pages-subtitle');
    const listEl = document.getElementById('preview-service-pages-list');
    ...
    if (!pages.length) {
        listEl.innerHTML = '<div class="preview-empty">No service pages added yet.</div>';
        return;
    }

    listEl.innerHTML = pages.map((page) => {
        const pageTitle = escapeHtml(page.title || 'Service');
        const slug = escapeHtml(page.slug || slugify(page.title || 'service'));
        const summary = escapeHtml(page.summary || '');
        return `
            <div class="preview-service-page-card">
                <strong>${pageTitle}</strong>
                <div class="preview-service-page-slug">${slug}</div>
                ${summary ? `<div class="preview-service-page-summary">${summary}</div>` : ''}
            </div>
        `;
    }).join('');
}
...
async function handleBrandLogoUpload(event) {
    ...
    try {
        ...
        showNotification('An error occurred while uploading the logo.', 'error');
    } finally {
        fileInput.value = '';
    }
}

async function handlePartnerLogoUpload(event) {
    const fileInput = event.target;
    const file = fileInput.files?.[0];
    if (!file) {
        return;
    ...
    try {
        ...
        const result = await response.json();
        const item = fileInput.closest('.partner-item');
        const logoInput = item?.querySelector('[data-field="logo_url"]');
        if (logoInput) {
            logoInput.value = result.url;
        }
